...
Twig.prototype.onKernelCompile = function(event, cb) {
	...
	this._registerCustomTags(event.container);
	//this._registerCustomFunctions(event.container);
	//this._registerCustomFilters(event.container);
 ...
 * @private
 */
Twig.prototype._registerCustomTags = function(container) {
	var tags = container.getTagsByName('twig.tag');
		...
		tags.forEach(function(tag) {
			...
			if (service) {
				...
				if (method && typeof service[method] === 'function') {
					...
					service[tag.getParameter('method')](container, TwigJS);

				} else if (typeof service.registerTag === 'function') {

					service.registerTag(container, TwigJS);
